...
import warnings
import requests
import os  # Sistem bilgilerini okumak için gerekli
from tqdm.auto import tqdm
from concurrent.futures import ThreadPoolExecutor, as_completed

warnings.filterwarnings('ignore')

# --- TELEGRAM AYARLARI (GitHub Secrets'tan Çeker) ---
TOKEN = os.getenv("TELEGRAM_TOKEN")
CHAT_ID = os.getenv("TELEGRAM_CHAT_ID")

def send_telegram_msg(message):
    if not TOKEN or not CHAT_ID:
        print("Hata: TOKEN veya CHAT_ID bulunamadı!")
        return
...
def process_ticker(ticker, df_h, df_d):
    try:
        ...
        if any([f1s, f2s, f4s]):
            kombo_score = sum([f1s, f2s, f4s])
            if kombo_score < 2 and last_close <= daily_200_sma:
                return None

            t_name = ticker.replace(".IS","")
            sma_status = "🟢" if last_close > daily_200_sma else "🔴"
            
            msg_line = (f"<b>{t_name}</b> | Fiyat: {last_close:.2f}\n"
                        f"BB Orta 200: {sma_status} | Sinyal: "
                        f"{'1S ' if f1s else ''}{'2S ' if f2s else ''}{'4S' if f4s else ''}\n")
            
            weight = (kombo_score * 1000) + (int(f4s)*100) + (int(f2s)*10) + (int(f1s)*1)
            return {"line": msg_line, "weight": weight}
    except: return None
...
if results:
    ...
    final_message = "🎯 <b>BIST RADAR ANALİZ SONUÇLARI</b>\n\n"
    for r in results:
        final_message += r['line'] + "\n"
    final_message += "\n⚠️ <b>YASAL UYARI:</b> Yatırım tavsiyesi değildir."
    
    send_telegram_msg(final_message)
    print("✅ Sonuçlar Telegram'a gönderildi!")
else:
