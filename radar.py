...
def process_ticker(ticker):
    try:
        ...
        # Filtre: Çift veya Full kombo ise SMA şartı yok. Tek sinyal ise SMA 200 üstü şart.
        if score < 2 and last_close <= daily_200_sma: return None

        t_name = ticker.replace(".IS","")
        sma_icon = "🟢" if last_close > daily_200_sma else "🔴"
        
        # Periyot detaylarını yazdır
        periyotlar = []
        if f1s: periyotlar.append("1S")
        if f2s: periyotlar.append("2S")
        if f4s: periyotlar.append("4S")
        periyot_text = ", ".join(periyotlar)
        
        line = f"<b>{t_name}</b> | Fiyat: {last_close:.2f}\nBB Orta 200: {sma_icon} | Onay: {periyot_text}"
...
# --- MESAJLAŞTIRMA (BAŞLIKLAR DÜZENLENDİ) ---
if full_list:
    msg_full = "🔥 <b>FULL KOMBO (KRİTİK)</b> 🔥\n\n" + "\n\n".join(full_list)
    send_telegram_msg(msg_full)
    time.sleep(1) # Mesajların karışmaması için kısa bekleme

if cift_list:
    msg_cift = "⭐ <b>ÇİFT SİNYAL (TAKİP)</b> ⭐\n\n" + "\n\n".join(cift_list)
    send_telegram_msg(msg_cift)
    time.sleep(1)

if tek_list:
    msg_tek = "🔍 <b>TEK SİNYAL</b> 🔍\n\n" + "\n\n".join(tek_list)
    send_telegram_msg(msg_tek)

if not (full_list or cift_list or tek_list):
